.. card::  :fas:`earth-americas` The World
   ...
   The :py:mod:`~novel_swarms.world` contains all the objects that will be simulated.

   .. card::  :fas:`users-viewfinder` Population

      The :py:attr:`~novel_swarms.world.World.World.population` is the collection of agents in the world.
      ...
      .. card::  :far:`user` Agent
         ...
         .. grid::
            ...
            .. grid-item-card::  :fas:`gamepad` Controller
                  ...
                  Each agent has a :py:mod:`~novel_swarms.agent.control`\ ler that
                  can control the agent's movement and act based on the sensor information
                  each ``step()`` of the simulation.

      .. card:: :fas:`users` Agent

         A :py:attr:`~novel_swarms.world.World.World.population` often has multiple agents, each of
         which can have a different type, controller, or set of sensors.

   .. card::  :fas:`draw-polygon`  World Objects (props)

         :py:mod:`~novel_swarms.world.objects` are a special type of agent that are not part of the population.
         It is used to represent objects in the world that are not agents, such as walls, props, and triggers.
         They are stored in the world's :py:attr:`~novel_swarms.world.World.World.objects` list.

   .. card::  :fas:`hands-holding-child` Spawners
         
         :py:mod:`~novel_swarms.world.spawners` can create new agents and
         add them to the :py:attr:`~novel_swarms.world.World.World.population`.
